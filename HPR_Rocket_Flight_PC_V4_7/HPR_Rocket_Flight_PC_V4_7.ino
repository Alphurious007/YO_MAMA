...
#define mySerial7 Serial7 // RFD900 Setup
String data = "";
...
void setup(void) {
  ...
  mySerial7.begin(57600); // RFD900
  delay(500);

  mySerial7.println("Connected!");
...
void loop(void){
  ...
  // if(events.touchdown){data += ("| TOUCHDOWN | ");}
  // else if(events.mainDeploy){data += ("| MAIN_DEPLOYED | ");}
  // else if(events.apogee){data += ("| APOGEE_REACHED | ");}
  // else if(events.liftoff){data += ("| LIFTOFF | ");}
  // else if(events.preLiftoff){data += ("| READY_FOR_LIFTOFF | ");}


  // data += ("Lat,Long=");
  // data += String(gnss.latitude,5);
  // data += (",");
  // data += String(gnss.longitude,5);
  // data += (" | "); 

  // data += ("B_Alt=");
  // data += String(baro.Alt);
  // data += (" | ");

  // data += ("B_Pres=");
  // data += String(baro.pressure);
  // data += (" | ");

  // data += ("B_Temp=");
  // data += String(baro.temperature);
  // data += (" | ");

  // data += ("A_Vel=");
  // data += String(accelVel);
  // data += (" | ");

  // data += ("B_Vel=");
  // data += String(baro.Vel);
  // data += (" | ");

  // data += ("F_Vel=");
  // data += String(fusionVel);
  // data += (" | ");

  // data += ("Cont_A,M=");
  // data += String(cont.apogee);
  // data += (",");
  // data += String(cont.main);
  // data += (" | "); 
  ...
  //if(settings.TXenable && sendPkt && !TX && sensors.statusRadio){
  if(settings.TXenable && sendPkt && !TX){
    sendTelemetryPkt();
