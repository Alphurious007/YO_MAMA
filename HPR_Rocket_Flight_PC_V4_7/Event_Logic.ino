...
void checkEvents(){
  ...
  // Variables to store previous baro.Vel value
  ...
  boolean baroApogee = (baro.Vel < -11 && baro.prevVel < -11 && accelVel < 70 && (baro.Alt + baro.baseAlt) < 9000) ? true : false;
  ...
  if (!events.apogee && events.boosterBurnout && !events.boosterBurnoutCheck && !pyroFire && baroApogee) {
    events.apogee = true;
  ...
  // Update the previous baro.Vel value
  baro.prevVel = baro.Vel;
