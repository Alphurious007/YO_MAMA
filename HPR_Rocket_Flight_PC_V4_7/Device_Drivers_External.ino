...
void getExternalAccel(){
  ...
  imu::Vector<3> accelvec = bno.getVector(Adafruit_BNO055::VECTOR_ACCELEROMETER);

  //set the sensor raw data equal to the raw sensor output
  accel.rawX = static_cast<int16_t>(accelvec.x()*100);
  accel.rawY = static_cast<int16_t>(accelvec.y()*100);
  accel.rawZ = static_cast<int16_t>(accelvec.z()*100);}
...
void getExternalGyro(){
  ...
  //set the sensor raw data equal to the raw sensor output
  gyro.rawX = static_cast<int16_t>(gyrovec.x());
  gyro.rawY = static_cast<int16_t>(gyrovec.y());
  gyro.rawZ = static_cast<int16_t>(gyrovec.z());}
...
bool beginExternalMag(){
 ...
 void getExternalMag(){
  ...
  //set the sensor raw data equal to the raw sensor output
  mag.rawX = static_cast<int16_t>(magvec.x());
  mag.rawY = static_cast<int16_t>(magvec.y());
  mag.rawZ = static_cast<int16_t>(magvec.z());}
